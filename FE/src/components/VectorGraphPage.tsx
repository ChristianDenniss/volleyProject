...
const vectorGraphPage =
  "p-[20px] max-w-[1200px] mx-auto min-h-[100vh] box-border bg-white upto-md:p-[1rem]";
...
const vectorGraphSubtitle = "text-[1rem] text-[#4a5568] leading-[1.5] m-0";
...
const vectorGraphControls =
  ...
  "bg-[#f7fafc] rounded-[8px] border border-[#e2e8f0] " +
...
const statValueLarge =
  "text-[18px] font-bold text-brand-primary leading-[1] m-0 py-[4px] px-0";
...
const controlSelect =
  "text-[14px] py-[6px] px-[12px] pr-[32px] border border-brand-primary rounded-[4px] " +
  "bg-brand-primary bg-[image:var(--chevron-down-white)] bg-no-repeat bg-[right_8px_center] bg-[length:20px] " +
  "text-white cursor-pointer transition-[background-color] duration-200 ease-[ease] " +
  "appearance-none shadow-none h-auto " +
  "hover:bg-brand-primary-hover hover:border-brand-primary-hover hover:outline-none " +
  "hover:shadow-[0_0_0_2px_rgba(45,60,80,0.2)] " +
  "focus:bg-brand-primary-hover focus:border-brand-primary-hover focus:outline-none " +
  "focus:shadow-[0_0_0_2px_rgba(45,60,80,0.2)]";

const controlSlider =
  "w-full h-[6px] rounded-[3px] bg-[#e2e8f0] outline-none appearance-none " +
  ...
  "[&::-webkit-slider-thumb]:rounded-[50%] [&::-webkit-slider-thumb]:bg-brand-primary " +
  ...
  "[&::-webkit-slider-thumb:hover]:bg-brand-primary-hover " +
  "[&::-moz-range-thumb]:w-[18px] [&::-moz-range-thumb]:h-[18px] " +
  "[&::-moz-range-thumb]:rounded-[50%] [&::-moz-range-thumb]:bg-brand-primary " +
  ...
  "[&::-moz-range-thumb]:ease-[ease] [&::-moz-range-thumb:hover]:bg-brand-primary-hover";

const sliderLabels = "flex justify-between text-[12px] text-[#718096] mt-[4px]";

const vectorGraphContent =
  "my-[20px] mx-0 bg-[#f7fafc] rounded-[8px] border border-[#e2e8f0] p-[1rem] " +
  "min-h-[600px] upto-md:min-h-[400px]";

const vectorGraphFooter =
  "my-[20px] mx-0 p-[20px] bg-[#f7fafc] rounded-[8px] border border-[#e2e8f0]";

const infoBoxTitle =
  "mt-0 mr-0 mb-[15px] ml-0 text-text text-[1.25rem] font-bold";

const infoBoxP = "leading-[1.6] text-[#4a5568] mb-[10px]";

/* `.info-box .version-info` (0,2,0) beats `.info-box p` for color and
   margin-bottom. line-height: 1.6 still comes from the p rule. */
const versionInfo = "leading-[1.6] text-[0.875rem] text-[#718096] italic mb-0";

const statusContainer =
  "text-center py-[4rem] px-[2rem] bg-[#f7fafc] rounded-[8px] border border-[#e2e8f0]";
...
const vectorGraphEmpty =
  "flex flex-col items-center justify-center h-[600px] text-[#718096] text-center p-[2rem] " +
  "upto-md:h-[400px] " +
  "[&_p]:my-[0.5rem] [&_p]:mx-0 [&_p]:text-[1rem] [&_p]:text-[#4a5568]";
...
const vectorGraphInfo =
  "absolute top-[1rem] right-[1rem] bg-[rgba(255,255,255,0.95)] rounded-[8px] p-[1rem] " +
  "min-w-[250px] max-w-[300px] min-h-[200px] shadow-[0_4px_6px_rgba(0,0,0,0.2)] z-[10] " +
  "max-h-[calc(100%_-_2rem)] overflow-y-auto border border-[#e2e8f0] " +
...
const infoBoxToggle =
  ...
  "text-[#4a5568] cursor-pointer p-0 w-[24px] h-[24px] flex items-center justify-center " +
  "leading-[1] transition-[color] duration-200 ease-[ease] z-[11] hover:text-text";

const infoBoxShowButton =
  "absolute top-[1rem] right-[1rem] bg-[rgba(255,255,255,0.95)] border border-[#e2e8f0] " +
  "rounded-[8px] text-[1.5rem] text-[#4a5568] cursor-pointer py-[0.5rem] px-[0.75rem] " +
  "w-[40px] h-[40px] flex items-center justify-center shadow-[0_4px_6px_rgba(0,0,0,0.2)] " +
  "transition-all duration-200 ease-[ease] z-[10] " +
  "hover:bg-[rgba(255,255,255,1)] hover:text-text hover:shadow-[0_6px_8px_rgba(0,0,0,0.3)]";

const infoSection =
  "mb-[1rem] border-b border-b-[#e2e8f0] last:border-b-0 last:mb-0 last:pb-0 " +
  "[&_p]:my-[0.25rem] [&_p]:mx-0 [&_p]:text-[0.85rem] [&_p]:text-[#4a5568] [&_p]:leading-[1.4] " +
  "[&_ul]:my-[0.5rem] [&_ul]:mx-0 [&_ul]:pl-[1.25rem] [&_ul]:text-[0.85rem] [&_ul]:text-[#4a5568] " +
...
const infoSectionHeaderTitle =
  "m-0 flex-1 pointer-events-none text-[0.95rem] text-[#4a5568] font-semibold";

const collapseIcon =
  "text-[0.85rem] text-[#718096] transition-[transform] duration-200 ease-[ease] " +
  "ml-[0.5rem] pointer-events-none inline-block min-w-[12px]";

const hoveredPlayerInfo =
  "bg-accent-border p-[0.75rem] rounded-[6px] mt-[0.5rem] border border-[#CFFFFF]";
...
const playerInfoHeaderTitle =
  "mt-0 mr-0 mb-[0.5rem] ml-0 flex-1 text-brand-primary text-[1rem] font-bold";
...
const clusterInfo =
  "mt-[0.5rem] pt-[0.5rem] border-t border-t-[rgba(0,0,0,0.1)]";
...
const VectorGraphPage: React.FC = () => {
    ...
    return (
      <div className={vectorGraphPage}>
        <div className={statusContainer}>
          <h2 className={statusTitle}>Loading Vector Graph...</h2>
  ...
  if (playersError || seasonsError) {
    return (
      <div className={vectorGraphPage}>
        <div className={statusContainer}>
          <h2 className={statusTitle}>Error Loading Data</h2>
          {playersError && <p className={errorText}>Error loading players: {playersError}</p>}
          {seasonsError && <p className={errorText}>Error loading seasons: {seasonsError}</p>}
        </div>
      </div>
    );
  }
